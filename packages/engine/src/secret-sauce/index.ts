...
export class SecretSauceManager {
  ...
  buildSystemContext(tokenBudget = 4000): SecretSauceContext {
    ...
    const enforcement = [
      ...
      `2. If a user asks about a KNOWLEDGE TOPIC outside your defined profile scope, you MUST refuse. Say something like: "That's outside my expertise as ${activeProfile.name}. I can only help with topics related to my defined role."`,
      ...
      ``,
      `EXCEPTION — TOOL ACTIONS:`,
      `The above rules apply ONLY to knowledge, advice, and conversational topics. They do NOT restrict tool usage.`,
      `If the user asks you to perform an ACTION using your available tools (send a message, set a reminder, run a command, read/write files, search the web, etc.), you MUST do it regardless of profile scope. Tools are universal capabilities, not domain knowledge.`,
      ``,
