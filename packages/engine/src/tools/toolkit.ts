...
const CORE_TOOLS: ToolDefinition[] = [
  ...
  // ═══ SHELL & PROCESS ═══
  { id: 'shell_exec', name: 'Execute Command', description: 'Run a shell command', modelDescription: 'Run a shell command when it is genuinely the fastest/only way to complete a step (npm/pnpm/build scripts, one-off CLIs like jq/curl/kubectl, or operations no dedicated tool handles). Prefer built-in tools (glob, grep, file_read, script_run, node_rpc, web_fetch) for searching, parsing, reading, or routine calculations. Never use shell_exec as the first/easy default when another tool fits.', category: 'shell_process', riskLevel: 'high', schema: { type: 'object', properties: { command: { type: 'string', description: 'Shell command' }, cwd: { type: 'string', description: 'Working directory (optional)' }, timeout: { type: 'number', description: 'Timeout ms (default: 30000, max: 600000)' }, maxLength: { type: 'number', description: 'Max output chars (default: 30000)' } }, required: ['command'] }, composable: true, source: 'builtin' },
  { id: 'bash', name: 'Bash (alias)', description: 'Run a shell command', modelDescription: 'Alias for shell_exec. Use only when shell_exec is genuinely the best option.', category: 'shell_process', riskLevel: 'high', schema: { type: 'object', properties: { command: { type: 'string', description: 'Shell command' }, cwd: { type: 'string', description: 'Working directory' } }, required: ['command'] }, composable: true, source: 'builtin' },
  { id: 'run_command', name: 'Run Command (alias)', description: 'Run a shell command', modelDescription: 'Alias for shell_exec. Use only when shell_exec is genuinely the best option.', category: 'shell_process', riskLevel: 'high', schema: { type: 'object', properties: { command: { type: 'string', description: 'Command' }, cmd: { type: 'string', description: 'Command (alias)' } }, required: [] }, composable: true, source: 'builtin' },
  { id: 'execute', name: 'Execute (alias)', description: 'Run a shell command', modelDescription: 'Alias for shell_exec. Use only when shell_exec is genuinely the best option.', category: 'shell_process', riskLevel: 'high', schema: { type: 'object', properties: { command: { type: 'string', description: 'Command' } }, required: ['command'] }, composable: true, source: 'builtin' },
  ...
  { id: 'shell_exec_streaming', name: 'Stream Command Output', description: 'Run command with streaming output', modelDescription: 'Stream a long-running shell command only when real-time output is needed and no built-in tool handles the task. Avoid for scraping/search/calculation.', category: 'shell_process', riskLevel: 'high', schema: { type: 'object', properties: { command: { type: 'string', description: 'Shell command' }, cwd: { type: 'string', description: 'Working directory (optional)' }, maxLength: { type: 'number', description: 'Max output chars (default: 30000)' } }, required: ['command'] }, composable: false, source: 'builtin' },
  ...
  { id: 'apply_patch', name: 'Apply Patch (alias)', description: 'Apply a patch to a file', modelDescription: 'Apply patch hunks or {search,replace} edits. *** Begin Patch format supported.', category: 'code_intelligence', riskLevel: 'medium', schema: { type: 'object', properties: { path: { type: 'string', description: 'File path' }, patch: { type: 'string', description: 'Patch text (Begin Patch format or +/- hunks)' }, edits: { type: 'array', description: 'Edits array (alternative)' } }, required: [] }, composable: true, source: 'builtin' },
  ...
  { id: 'agent_x_overview', name: 'Agent-X Overview', description: 'Fleet-wide snapshot of sessions, automations, notifications, and settings', modelDescription: 'Agent-X core and messaging channel sessions. Returns live Agent-X state: summary (default), sessions, automations, notifications, settings, or session_detail (requires session_id). Use before answering questions about channel connectivity, other sessions, automations, private chats, or configuration — never invent "Telegram not connected" without checking.', category: 'agent_meta', riskLevel: 'low', schema: { type: 'object', properties: { view: { type: 'string', enum: ['summary', 'sessions', 'automations', 'notifications', 'settings', 'session_detail'], description: 'Which snapshot to return (default summary)' }, session_id: { type: 'string', description: 'Required when view=session_detail' } }, required: [] }, composable: true, source: 'builtin' },
  ...
  { id: 'markdown_list', name: 'List Markdown Documents', description: 'List saved markdown documents from the sidebar', modelDescription: 'List saved markdown documents from the Agent-X sidebar (NOT the filesystem). These are documents created via save_to_markdown — reports, itineraries, plans, etc. Use when the user asks to "list markdowns", "show saved documents", "what reports do I have", etc. Returns title, id, and creation date for each. Pass session_only=true to limit to the current session.', category: 'documents', riskLevel: 'low', schema: { type: 'object', properties: { limit: { type: 'number', description: 'Max results (1-200, default 50)' }, session_only: { type: 'boolean', description: 'Only list documents from the current session (default: false — list all)' } }, required: [] }, composable: true, source: 'builtin' },
  ...
  { id: 'notify_telegram', name: 'Telegram Notification', description: 'Send a Telegram message', modelDescription: 'Send a notification/message via Telegram bot. Bot token and chat ID are auto-loaded from Settings → Channels; do not ask the user for them.', category: 'communication', riskLevel: 'low', schema: { type: 'object', properties: { message: { type: 'string', description: 'Message text' } }, required: ['message'] }, composable: true, source: 'builtin' },
  { id: 'notify_slack', name: 'Slack Notification', description: 'Send a Slack message', modelDescription: 'Send a notification/message via Slack webhook. The webhook URL is auto-loaded from Settings → Channels; do not ask the user for it.', category: 'communication', riskLevel: 'low', schema: { type: 'object', properties: { message: { type: 'string', description: 'Message text' } }, required: ['message'] }, composable: true, source: 'builtin' },
  { id: 'notify_email', name: 'Email Notification', description: 'Send an email notification', modelDescription: 'Send a notification via configured SMTP. SMTP credentials are auto-loaded from Settings → Channels; do not ask the user for them.', category: 'communication', riskLevel: 'low', schema: { type: 'object', properties: { message: { type: 'string', description: 'Message body' }, subject: { type: 'string', description: 'Email subject' } }, required: ['message'] }, composable: true, source: 'builtin' },
  { id: 'notify_discord', name: 'Discord Notification', description: 'Send a Discord message', modelDescription: 'Send a notification via Discord webhook. The webhook URL is auto-loaded from Settings → Channels; do not ask the user for it.', category: 'communication', riskLevel: 'low', schema: { type: 'object', properties: { message: { type: 'string', description: 'Message text' } }, required: ['message'] }, composable: true, source: 'builtin' },
  ...
  { id: 'telegram_send_message', name: 'Send Telegram Message', description: 'Send a text message back to the active Telegram chat', modelDescription: 'Send a text message to the user on the active Telegram chat. Use this for replies, progress updates, or follow-up questions during a Telegram session. The chat_id can be omitted when replying to an active inbound message or when a default chat is configured in Settings → Channels.', category: 'communication', riskLevel: 'low', schema: { type: 'object', properties: { message: { type: 'string', description: 'The message text to send' }, chat_id: { type: 'string', description: 'Optional Telegram chat id (defaults to the active inbound chat or configured default)' } }, required: ['message'] }, composable: true, source: 'builtin' },
  { id: 'telegram_send_file', name: 'Send Telegram File', description: 'Upload and send a file to the active Telegram chat', modelDescription: 'Upload and send a file to the active Telegram chat. Use this when the user asks for a file, report, PDF, document, etc. The file must exist on disk. The chat_id can be omitted when replying to an active inbound message or when a default chat is configured in Settings → Channels.', category: 'communication', riskLevel: 'medium', schema: { type: 'object', properties: { path: { type: 'string', description: 'Path to the file to send' }, caption: { type: 'string', description: 'Optional caption/description for the file' }, chat_id: { type: 'string', description: 'Optional Telegram chat id (defaults to the active inbound chat or configured default)' } }, required: ['path'] }, composable: true, source: 'builtin' },
  ...
  { id: 'discord_send_message', name: 'Send Discord Message', description: 'Send a text message back to the active Discord channel or DM', modelDescription: 'Send a text message to the active Discord channel or DM. Use this for replies, progress updates, or follow-up questions during a Discord session. The channel_id can be omitted when replying to an active inbound message or when a default channel is configured in Settings → Channels.', category: 'communication', riskLevel: 'low', schema: { type: 'object', properties: { message: { type: 'string', description: 'The message text to send' }, channel_id: { type: 'string', description: 'Optional Discord channel id (defaults to the active inbound channel or configured default)' } }, required: ['message'] }, composable: true, source: 'builtin' },
  { id: 'discord_send_file', name: 'Send Discord File', description: 'Upload and send a file to the active Discord channel or DM', modelDescription: 'Upload and send a file to the active Discord channel or DM. Use this when the user asks for a file, report, PDF, document, etc. The file must exist on disk. The channel_id can be omitted when replying to an active inbound message or when a default channel is configured in Settings → Channels.', category: 'communication', riskLevel: 'medium', schema: { type: 'object', properties: { path: { type: 'string', description: 'Path to the file to send' }, message: { type: 'string', description: 'Optional message text to include with the file' }, channel_id: { type: 'string', description: 'Optional Discord channel id (defaults to the active inbound channel or configured default)' } }, required: ['path'] }, composable: true, source: 'builtin' },
  { id: 'email_send_message', name: 'Send Email Message', description: 'Send an email reply or message via the active email channel', modelDescription: 'Send an email message via the active email session. Use this for replies, reports, or follow-ups. The to address can be omitted when replying to an active inbound email or when a default recipient is configured in Settings → Channels.', category: 'communication', riskLevel: 'low', schema: { type: 'object', properties: { message: { type: 'string', description: 'The plain-text email body' }, subject: { type: 'string', description: 'Email subject' }, to: { type: 'string', description: 'Optional recipient address (defaults to the active inbound sender or configured default)' } }, required: ['message'] }, composable: true, source: 'builtin' },
  { id: 'email_send_file', name: 'Send Email Attachment', description: 'Send an email with an attachment via the active email channel', modelDescription: 'Send an email with an attachment via the active email session. Use this when the user asks for a file, report, PDF, document, etc. The file must exist on disk. The to address can be omitted when replying to an active inbound email or when a default recipient is configured in Settings → Channels.', category: 'communication', riskLevel: 'medium', schema: { type: 'object', properties: { path: { type: 'string', description: 'Path to the file to send' }, body: { type: 'string', description: 'Optional plain-text email body' }, subject: { type: 'string', description: 'Email subject' }, to: { type: 'string', description: 'Optional recipient address (defaults to the active inbound sender or configured default)' } }, required: ['path'] }, composable: true, source: 'builtin' },

  // ═══ AGENT META-TOOLS ═══
  { id: 'todo_write', name: 'Write Todos', description: 'Create or update session task list', modelDescription: 'Your turn-scoped execution plan (not a UI notification). REQUIRED for non-trivial missions. merge:false replaces all; merge:true updates by id (preferred) or exact content title. Mark active work in_progress — multiple in_progress items are allowed for parallel streams. After EACH completed step, immediately todo_write(merge:true) marking completed and advancing next pending item(s) to in_progress. Never leave the first item stuck in_progress after you moved on. Status: pending|not-started, in_progress, completed. Trust [ACTIVE_TODOS] / mid-turn checklist updates as source of truth.', category: 'agent_meta', riskLevel: 'low', schema: { type: 'object', properties: { merge: { type: 'boolean', description: 'Merge with existing (default: false)' }, todos: { type: 'array', description: 'Array of {id?, content, status?}' } }, required: ['todos'] }, composable: false, source: 'builtin' },
  { id: 'todo_read', name: 'Read Todos', description: 'List session tasks', modelDescription: 'Read the live checklist. Prefer the [ACTIVE_TODOS] prompt section / mid-turn updates when present; call this if you need to re-sync after a long tool sequence.', category: 'agent_meta', riskLevel: 'low', schema: { type: 'object', properties: {}, required: [] }, composable: true, source: 'builtin' },
  ...
  { id: 'delegate_to_subagent', name: 'Delegate to Sub-Agent', description: 'Spawn a specialist sub-agent to handle a parallel task', modelDescription: 'Delegate work to an isolated sub-agent with its own tools/memory. Prefer multiple delegate_to_subagent calls in ONE step for parallelism — the parent WAITS for each result and continues the turn . Use "mission" for a single task, or "items"+"batchSize" for batch fan-out. Set background:true ONLY for messaging-channel notify-later jobs; on desktop chat omit it so you receive the child result and can merge/finish.', category: 'agent_meta', riskLevel: 'low', schema: { type: 'object', properties: { mission: { type: 'string', description: 'Clear, specific mission. Use for single tasks. Ignored if "items" is provided.' },           items: { type: 'array', items: { type: 'string', description: 'An item to process' }, description: 'Array of items to process in parallel. Each item gets its own sub-agent (or chunked by batchSize).' }, batchSize: { type: 'number', description: 'Number of items per sub-agent (default: 10). Only used with "items".' }, tools: { type: 'array', description: 'Optional list of tool IDs the sub-agent can use. If omitted, all tools are available.' }, timeout: { type: 'number', description: 'Timeout in milliseconds per sub-agent (default: 600000)' }, background: { type: 'boolean', description: 'Messaging-channel notify-later only. On desktop the parent still waits. Default false.' } }, required: [] }, composable: false, source: 'builtin' },
...
function legacyTemplateShim(replacementId: string) {
  return async (_args: Record<string, unknown>, _context: any): Promise<any> => ({
    success: false,
    output: `Legacy tool retired. Use ${replacementId} instead.`,
    error: 'DEPRECATED',
  });
}
...
export function createDefaultToolkit(
  scopePath: string,
  config?: { documentStudioTools?: DocumentStudioToolsConfig },
  ...
  executor.registerHandler('gen_markdown', documents.docMarkdown);
  executor.registerHandler('gen_html', documents.docHtml);
  executor.registerHandler('gen_json', documents.docJson);
  executor.registerHandler('gen_yaml', documents.docYaml);
  executor.registerHandler('gen_diagram', documents.docDiagram);
  executor.registerHandler('gen_latex', documents.docLatex);
  ...
  // ═══ Legacy template_* cutover (Document Studio §9.7.8) ═══
  const legacyMode = config?.documentStudioTools?.legacyTemplateTools;
  if (legacyMode === false || legacyMode === 'off') {
    for (const legacyId of ['template_list', 'template_inspect', 'template_fill']) {
      registry.unregister(legacyId);
      executor.unregisterHandlersByPrefix(legacyId);
    }
  } else if (legacyMode === 'shim') {
    executor.registerHandler('template_list', legacyTemplateShim('doc_master_list'));
    executor.registerHandler('template_inspect', legacyTemplateShim('doc_master_get'));
    executor.registerHandler('template_fill', legacyTemplateShim('doc_job_run'));
  }
  ...
  executor.registerHandler('markdown_list', markdownTool.markdownList);
