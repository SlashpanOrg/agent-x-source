...
function GitOperationCardImpl({ tool, defaultExpanded = false }: { tool: ToolCall; defaultExpanded?: boolean }) {
  ...
  return (
    <Box sx={{
      ...
      >
        <Box sx={{ color: isRunning ? colors.accent.blue : isError ? colors.accent.red : colors.accent.purple, display: 'flex', alignItems: 'center', opacity: 0.7 }}>
          <MergeTypeIcon sx={{ fontSize: 12 }} />
        </Box>
